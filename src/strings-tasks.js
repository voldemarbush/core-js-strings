...
function isString(value) {
  return Object.prototype.toString.call(value).search(/String/) > 1;
}
...
function removeFirstOccurrences(str, value) {
  ...
  return index >= 0
    ? str.substring(0, index).concat(str.substring(index + value.length))
    : str;
}
...
function countVowels(str) {
  ...
  return vowels.reduce(
    (acc, item) => acc + (str.match(new RegExp(item, 'g')) || '').length,
    0
  );
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((item) => {
      ...
      return String.fromCharCode(charCode + codeShift);
    })
    .join('');
}
...
function getCardId(value) {
  ...
  return value
    ...
    .reduce(
      (acc, item) =>
        acc + +item.replace(/[AJQK♥♦♣♠]/, (match) => symbols[match]),
      -1
    );
}
